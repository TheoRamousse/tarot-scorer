...
namespace Model
{
    public class FrenchTarotRules : IRules
    {
        ...
        {
            if(!CheckNbPlayers(game, out validity))
                return false;
            if(!CheckBiddings(game, out validity))
                return false;
            //if(!CheckPoints(game, out validity))
            //    return false;
            return true;
        }
        ...
        public int MinNbPlayersForKingCalled { get; } = 5;

        public int MaxNbKingsCalled { get; } = 1;
        ...
        private bool CheckTaker(Game game, out Validity validity)
        {
            ...
            if(nbTakers == 0)
            {
                validity = Validity.NoTaker;
                return false;
            }
            if(nbTakers > 1)
            {
                validity = Validity.TooManyTakers;
                return false;
            }
            validity = Validity.Valid;
            return true;
        ...
        private bool CheckKingCalled(Game game, out Validity validity)
        {
            ...
            if(game.NbPlayers < MinNbPlayersForKingCalled
                && nbKingsCalled > 0)
            {
                validity = Validity.ShouldNotHaveKingCalled;
                return false;
            }
            if(game.NbPlayers >= MinNbPlayersForKingCalled
                && nbKingsCalled > MaxNbKingsCalled)
            {
                validity = Validity.TooManyKingsCalled;
                return false;
            }
            validity = Validity.Valid;
            return true;
        ...
        {
            if(!CheckTaker(game, out validity)
                || !CheckKingCalled(game, out validity))
                return false;

            if(game.Players.Where(r => r.Value == Bidding.None).Count() > 0)
            {
                validity = Validity.PlayerShallHaveBidding;
                return false;
            }
            validity = Validity.Valid;
            return true;
